...
import { BIND_USER_BASIC_INFO, CLEAR_USER_ALL_STATE, GET_USERS_BY_QUERY, GET_USER_BY_ID, GET_USER_DROPDOWN, IS_USER_ADD_FORM_SIDEBAR_OPEN, IS_USER_DATA_LOADED, IS_USER_DATA_PROGRESS, IS_USER_DATA_SUBMIT_PROGRESS, IS_USER_EDIT_FORM_SIDEBAR_OPEN, IS_USER_PHOTO_UPLOADED } from "../action-types";
...
export const isUserPhotoOnProgress = ( condition ) => dispatch => {
    dispatch( {
        type: IS_USER_PHOTO_UPLOADED,
        isUserPhotoUploadProgress: condition
    } );
};
...
export const userImageUpload = ( file, formData ) => async ( dispatch, getState ) => {
    dispatch( isUserPhotoOnProgress( true ) );
    ...
    await baseAxios.post( path, formData )
        .then( response => {
            ...
            if ( response.status === status.success ) {
                ...
                dispatch( isUserPhotoOnProgress( false ) );
            }
        } ).catch( ( { response } ) => {
