/**
 * Title: Production Sub Process List
 * Description: Production Sub Process List
 * Author: Nasir Ahmed
...
import ProductionSubProcessForm from '../forms/ProductionSubProcessForm';
import { fetchProductionSubProcessesByQuery, toggleProductionSubProcessSidebar } from '../store/actions';
import { productionSubProcessColumn } from './productionSubprocessColum';

const ProductionSubProcessList = () => {
  //#region Hooks
  const dispatch = useDispatch();
  const { items, loading, total, isOpenSidebar, selectedItem } = useSelector( ( { productionSubProcessReducer } ) => productionSubProcessReducer );
  ...
  //#region UDFs
  /**
   * Get All Production Sub Process
   */
  const getAllProductionSubProcess = () => {
    dispatch(
      fetchProductionSubProcessesByQuery( {
        page: currentPage,
        perPage: rowsPerPage,
  ...
  * For Open Sidebar
  */
  const toggleSidebar = () => dispatch( toggleProductionSubProcessSidebar( !isOpenSidebar ) );
  /**
    * For Filter
    */
  ...
  */
  ...
  return (
    <div>
        ...
        <FormLayout isNeedTopMargin={true} >
          ...
          >
            <IconButton
              ...
              color='primary'
              classNames="ml-1"
              onClick={() => getAllProductionSubProcess()}
          ...
          <CustomDataTable
            ...
            columns={productionSubProcessColumn}
            data={items}
      ...
      {isOpenSidebar && <ProductionSubProcessForm selectedItem={selectedItem} lastPageInfo={{ page: currentPage, perPage: rowsPerPage, status: currentStatus?.value }} />}
    </div>
  );
};

export default ProductionSubProcessList;
